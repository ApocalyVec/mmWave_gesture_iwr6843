...
if __name__ == '__main__':
    ...
    label_dict_path = 'D:/alldataset/thm_dataset_ts_5/thm_label_dict.p'
    data_without_label = pickle.load(open('D:/alldataset/thm_dataset_ts_5/thm_data_without_label.p', 'rb'))
    ...
    es = EarlyStopping(monitor='val_loss', mode='min', verbose=1, patience=50)
    mc = ModelCheckpoint(
        'D:/trained_models/bestSoFar_thm_CNN' + str(datetime.datetime.now()).replace(':', '-').replace(
            ' ', '_') + '.h5',
        monitor='val_loss', mode='min', verbose=1, save_best_only=True)
